...
public class TestPackedInts extends LuceneTestCase {
    ...
    private static void assertListEquality(
            List<? extends Reader> packedInts) {
        assertListEquality("", packedInts);
    }

    private static void assertListEquality(
            String message, List<? extends Reader> packedInts) {
        if (packedInts.size() == 0) {
            return;
        }
        Reader base = packedInts.get(0);
        int valueCount = base.size();
        for (Reader packedInt : packedInts) {
    ...
    public void testPackedInts() throws IOException {
        ...
        for (int iter = 0; iter < num; iter++) {
            for (int nbits = 1; nbits <= 64; nbits++) {
                ...
                {// test reader
                    IndexInput in = d.openInput("out.bin", newIOContext(random()));
                    Reader r = PackedInts.getReader(in);
                ...
                { // test direct reader get
                    IndexInput in = d.openInput("out.bin", newIOContext(random()));
                    Reader intsEnum = PackedInts.getDirectReader(in);
                    for (int i = 0; i < valueCount; i++) {
        ...
        for (int version = PackedInts.VERSION_START; version <= PackedInts.VERSION_CURRENT; ++version) {
            for (int bpv = 1; bpv <= 64; ++bpv) {
                for (PackedInts.Format format : PackedInts.Format.values()) {
                    ...
                    // test direct reader
                    in.seek(0L);
                    final Reader directReader = PackedInts.getDirectReaderNoHeader(in, format, version, valueCount, bpv);
